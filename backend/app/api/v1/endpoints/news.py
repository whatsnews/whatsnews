...
from app.schemas.news import NewsCreate, News as NewsSchema, NewsUpdate, NewsResponse
...
async def get_news(
    *,
    ...
    frequency: Optional[str] = Query(None, regex="^(hourly|daily)$"),
    ...
    if frequency:
        try:
            freq_value = UpdateFrequency(frequency.lower())
...
async def get_latest_news(
    prompt_id: int,
    frequency: str = Query(..., regex="^(hourly|daily)$"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user)
) -> Any:
